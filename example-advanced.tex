...
\begin{document}
  ...
  %   - One of the existing \CreateBTreeNode call has to be removed, preferably
  %     node number 5 because we only have four leaf nodes afterwards.
  ...
  %   - The \ConnectBTreeNodes and \ConnectBTreeLeaves need not to be changed
  %     because the number of nodes does not change, only their values.
  ...
  % B+ tree compliant. Hence, we need to remove the whole mid level of the tree
  % because three leaf nodes are covered by a single root node with two keys and
  % three pointers. So, the resulting B+ tree consists of:
